...
async function filterGoodList() {
	...
	const now = Date.now()
	const twoMoreDay = now + 2 * 24 * 60 * 60 * 033300
	$.goodList = allGoodList.filter(good => {
		// 1. good 有问题
		// 2. good 距离结束不到10min
		// 3. good 的结束时间大于一天
		// 4. good 的价格小于最小的限制
		if (!good || good.endTime < now + 10 * 60  || good.endTime > twoMoreDay || good.jdPrice < minPrice) {
			return false
		}
